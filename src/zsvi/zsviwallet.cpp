...
CzSVIWallet::CzSVIWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsvi seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSVISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSVISeed_deprecated()) {
                    LogPrintf("%s: Updated zSVI seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsvi seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsvi wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSVIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSVICount(nCountLastUsed);
    else if (!walletdb.ReadZSVICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSVIWallet::Lock()
{
    seedMaster = 0;
}

void CzSVIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSVIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSVI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSVIWallet::LoadMintPoolFromDB()
{
...
void CzSVIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSVIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSVIWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsviTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSVIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSVI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsviTracker which also adds to database
    pwalletMain->zsviTracker->Add(dMint, true);
...
void CzSVIWallet::SeedToZSVI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSVIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSVIWallet::GenerateDeterministicZSVI(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSVIWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSVIWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSVIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
