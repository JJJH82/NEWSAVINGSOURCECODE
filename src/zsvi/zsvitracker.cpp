...
CzSVITracker::CzSVITracker(std::string strWalletFile)
{
...
CzSVITracker::~CzSVITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSVITracker::Init()
{
...
bool CzSVITracker::Archive(CMintMeta& meta)
{
...
bool CzSVITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSVITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSVITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSVITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSVITracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSVITracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzSVITracker::GetSerialHashes()
{
...
CAmount CzSVITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSVITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSVITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSVITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSVITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSVITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSVITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSVITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSVITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSVITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSVITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSVIWallet* zSVIWallet)
{
    bool iszSVIWalletInitialized = (NULL != zSVIWallet);
    CMintMeta meta;
    ...
    if (! iszSVIWalletInitialized)
        zSVIWallet = new CzSVIWallet(strWalletFile);
    meta.isSeedCorrect = zSVIWallet->CheckSeed(dMint);
    if (! iszSVIWalletInitialized)
        delete zSVIWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSVITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSVITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSVITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSVITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSVITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSVITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSVIWallet* zSVIWallet = new CzSVIWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zSVIWallet);
        }
        delete zSVIWallet;
        LogPrint("zero", "%s: added %d dzsvi from DB\n", __func__, listDeterministicDB.size());
    }
