...
public:
    ...
    bool WriteZSVISeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSVISeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSVISeed_deprecated(uint256& seed);
    bool EraseZSVISeed();
    bool EraseZSVISeed_deprecated();

    bool WriteZSVICount(const uint32_t& nCount);
    bool ReadZSVICount(uint32_t& nCount);
