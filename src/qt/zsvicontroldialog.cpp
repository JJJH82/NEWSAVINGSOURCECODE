...
#include "ui_zsvicontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zsviTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zsviTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSVI.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSVI seed used to mint this zSVI is not the same as currently hold in the wallet";
            else
