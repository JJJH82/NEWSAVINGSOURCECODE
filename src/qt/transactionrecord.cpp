...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZSVIModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zsviTracker->HasMintTx(hash))) {
            //zSVI stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZSVI;
...
bool IsZSVIType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZSVI:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZSVI || type == TransactionRecord::MNReward) {
