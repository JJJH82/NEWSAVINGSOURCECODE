...
// The zSVI block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zSVI is a hash of the serial
...
{
    //Create an output returning the zSVI that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZSVIOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zSVI output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zSVI", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZSVIOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zSVI output", __func__);
...
{
    CzSVITracker* zsviTracker = pwallet->zsviTracker.get();
    CMintMeta meta;
    if (!zsviTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zsviTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a SVI stake is the outpoint
