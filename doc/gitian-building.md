...
*Setup instructions for a gitian build of Saving using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Saving
...
- [Building Saving](#building-saving)
...
Clone the git repositories for saving and gitian and then checkout the saving version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/saving-crypto/saving.git
cd saving
...
Follow the instructions in [doc/release-process.md](release-process.md) in the saving repository
...
To build Saving (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the saving repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/saving/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/saving-crypto/saving
...
For example:
```bash
URL=https://github.com/crowning-/saving.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit saving=${COMMIT} --url saving=${URL} ../saving/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit saving=${COMMIT} --url saving=${URL} ../saving/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit saving=${COMMIT} --url saving=${URL} ../saving/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/saving-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/saving-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/saving-build.assert
```
...
[saving/gitian.sigs](https://github.com/saving-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
